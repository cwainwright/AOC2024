...
line_safety pairs =
  let safe_direction_pairs = map (\(a, b) -> (pair_differences a b, pair_direction a b)) pairs
   in all (\(safety, _) -> safety) safe_direction_pairs -- Check Safe DistancesB
...
line_variations line =
  map
    ( \i ->
        let (before, after) = splitAt i line
         in take (i - 1) before ++ after
    )
    [1 .. length line]
